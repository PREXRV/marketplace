  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'order_delivered':
        return <Package className="w-5 h-5 md:w-6 md:h-6 text-blue-500" />;
      case 'product_in_stock':
        return <Check className="w-5 h-5 md:w-6 md:h-6 text-green-500" />;
      case 'product_on_sale':
      case 'new_sale':
        return <Sparkles className="w-5 h-5 md:w-6 md:h-6 text-orange-500" />;
      default:
        return <Bell className="w-5 h-5 md:w-6 md:h-6 text-gray-500" />;
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-4 md:py-8">
          <Link
            href="/profile"
            className="inline-flex items-center gap-1.5 md:gap-2 text-primary hover:underline mb-4 md:mb-6 text-sm md:text-base"
          >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2 flex items-center gap-2 md:gap-3 flex-wrap">
                <Bell className="w-6 h-6 md:w-8 md:h-8 text-primary" />
                Уведомления
                {unreadCount > 0 && (
                  <span className="bg-red-500 text-white text-xs md:text-sm px-2 py-0.5 md:px-3 md:py-1 rounded-full">
                    {unreadCount} новых
                  </span>
                )}
              </h1>
              <p className="text-sm md:text-base text-gray-600">Следите за важными событиями</p>
            </div>

            {unreadCount > 0 && (
              <button
                onClick={handleMarkAllAsRead}
                className="btn-primary flex items-center justify-center gap-2 text-sm md:text-base px-4 py-2 md:px-6 md:py-3 w-full md:w-auto"
              >
          ...
          <div className="flex gap-2 md:gap-3 mb-4 md:mb-6">
            <button
              ...
              className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-medium transition text-sm md:text-base ${
                filter === 'all'
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl shadow-md p-8 md:p-12 text-center">
              ...
              <h3 className="text-lg md:text-xl font-semibold mb-2">Нет уведомлений</h3>
            ...
            <div className="space-y-3 md:space-y-4">
              {notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`bg-white rounded-xl shadow-md p-4 md:p-6 transition ${
                  ...
                  <div className="flex gap-3 md:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2">
                        <div>
                          <h3 className="font-bold text-base md:text-lg text-gray-900 mb-1 break-words">
                            {notification.title}
                          </h3>
