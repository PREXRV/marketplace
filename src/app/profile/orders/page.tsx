...
export default function OrdersPage() {
  ...
  const getPaymentStatusBadge = (order: Order) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border ${info.cls}`}>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-yellow-50 rounded-xl p-4 shadow-sm border-2 border-yellow-100">
              <p className="text-sm text-yellow-700 mb-1">Ожидают</p>
              <p className="text-2xl font-bold text-yellow-700">{stats.pending}</p>
            </div>
            <div className="bg-cyan-50 rounded-xl p-4 shadow-sm border-2 border-cyan-100">
              <p className="text-sm text-cyan-700 mb-1">Подтверждены</p>
              <p className="text-2xl font-bold text-cyan-700">{stats.confirmed}</p>
            </div>
            <div className="bg-blue-50 rounded-xl p-4 shadow-sm border-2 border-blue-100">
              <p className="text-sm text-blue-700 mb-1">В процессе</p>
              <p className="text-2xl font-bold text-blue-700">{stats.in_progress}</p>
            </div>
            <div className="bg-green-50 rounded-xl p-4 shadow-sm border-2 border-green-100">
              <p className="text-sm text-green-700 mb-1">Доставлены</p>
              <p className="text-2xl font-bold text-green-700">{stats.delivered}</p>
            </div>
            <div className="bg-orange-50 rounded-xl p-4 shadow-sm border-2 border-orange-100">
              <p className="text-sm text-orange-700 mb-1">Ожидает отмены</p>
              <p className="text-2xl font-bold text-orange-700">{stats.awaiting_cancellation}</p>
            </div>
            <div className="bg-red-50 rounded-xl p-4 shadow-sm border-2 border-red-100">
              <p className="text-sm text-red-700 mb-1">Отменены</p>
          ...
          <div className="space-y-6">
            {filteredOrders.map(order => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 border-gray-100 hover:border-primary/20"
                >
                  {/* Заголовок заказа */}
                  <div className="p-6 bg-gradient-to-r from-gray-50 to-white border-b-2 border-gray-100">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <h3 className="text-xl font-bold text-gray-900">Заказ №{order.order_number}</h3>
                              {/* Статус заказа + статус оплаты рядом */}
                              <div className="flex items-center flex-wrap gap-2 mt-1">
                                <span
                                  className={`inline-block px-4 py-1.5 rounded-full text-xs font-semibold border-2 ${getStatusColor(order.status)}`}
                                >
                      ...
                      {/* Сумма заказа */}
                      <div className="text-right bg-white px-6 py-4 rounded-xl border-2 border-primary/20">
                        <p className="text-sm text-gray-600 mb-1 font-medium">Итого к оплате</p>
                  ...
                  {/* ✅ Баннер: не оплачен онлайн-заказ */}
                  {isOnlineUnpaid && (
                    <div className="px-6 py-4 bg-yellow-50 border-b-2 border-yellow-200 flex items-center justify-between gap-4 flex-wrap">
                      <div className="flex items-center gap-3">
                        <span className="text-2xl">⚠️</span>
                        <div>
                          <p className="font-semibold text-yellow-800">Заказ не оплачен</p>
                          <p className="text-sm text-yellow-700">Оплатите заказ, чтобы мы начали его обработку</p>
                        </div>
                      </div>
                      <button
                        onClick={() => handleRetryPayment(order)}
                        disabled={retryingPayment === order.id}
                        className="px-6 py-2.5 bg-yellow-500 hover:bg-yellow-600 text-white font-semibold rounded-xl transition disabled:opacity-50 flex items-center gap-2 shadow-md"
                      >
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div className="bg-gray-50 rounded-xl p-4 border-2 border-gray-100">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="font-semibold text-gray-700">Получатель</p>
                        </div>
                        <p className="font-bold text-gray-900">{order.customer_name || 'Не указано'}</p>
                        <p className="text-gray-900">{order.customer_phone || 'Телефон не указан'}</p>
                      </div>
                      ...
                      <div className="bg-blue-50 rounded-xl p-4 border-2 border-blue-100">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="font-semibold text-blue-700">Доставка</p>
                        </div>
                        <p className="font-bold text-blue-900">{getDeliveryTypeText(order)}</p>
                        {order.delivery_city && (
                      ...
                      <div className="bg-green-50 rounded-xl p-4 border-2 border-green-100">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="font-semibold text-green-700">Оплата</p>
                        </div>
                        <p className="font-bold text-green-900">{getPaymentTypeText(order)}</p>
                        {/* ✅ Статус оплаты в карточке */}
                        {getPaymentStatusBadge(order) && (
                          <div className="mt-2">{getPaymentStatusBadge(order)}</div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="mb-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border-2 border-green-200">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm text-green-700 font-medium">Применён промокод</p>
                              <p className="text-lg font-bold text-green-900">{order.promo_code.code}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-sm text-green-700">Скидка</p>
                      ...
                      <div className="mt-6 space-y-6 animate-fadeIn">
                        {order.items && order.items.length > 0 && (
                          <div>
                            <div className="flex items-center gap-2 mb-4">
                              ...
                              <h4 className="font-bold text-lg">Товары в заказе ({order.items.length})</h4>
                            </div>
                            ...
                            <div className="space-y-3">
                              {order.items.map(item => {
                                ...
                                return (
                                  <div
                                    key={item.id}
                                    className={`flex items-center gap-4 p-4 rounded-xl transition border relative ${
                                      isFreeReward
                                  ...
                                  >
                                    {item.return_status && (
                                      <div className="absolute top-2 right-2 z-10">
                                        {item.return_status.status === 'pending' && (
                                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-500 text-white text-xs font-bold rounded-full shadow-lg">⏳ Возврат на рассмотрении</span>
                                        )}
                                        {item.return_status.status === 'approved' && (
                                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-full shadow-lg">✅ Возврат одобрен</span>
                                        )}
                                        {item.return_status.status === 'rejected' && (
                                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-red-500 text-white text-xs font-bold rounded-full shadow-lg">❌ Возврат отклонён</span>
                                        )}
                                        {item.return_status.status === 'need_more_info' && (
                                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-500 text-white text-xs font-bold rounded-full shadow-lg">📋 Нужна доп. информация</span>
                                        )}
                                        {item.return_status.status === 'completed' && (
                                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-gray-500 text-white text-xs font-bold rounded-full shadow-lg">✔️ Возврат завершён</span>
                                        )}
                                      </div>
                                    )}

                                    <div className={`w-20 h-20 rounded-lg flex-shrink-0 overflow-hidden border-2 ${isFreeReward ? 'border-emerald-200' : 'border-gray-200'} bg-white`}>
                                      {item.product_image ? (
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p className="font-semibold text-gray-900 mb-1 truncate">{item.product_name}</p>
                                      <div className="flex items-center gap-4 text-sm text-gray-600">
                                    ...
                                    <div className="text-right">
                                      {isFreeReward ? (
                                        <p className="text-lg font-bold text-emerald-600">Бесплатно</p>
                                      ) : (
                            ...
                            <div className="mt-4 p-4 bg-gray-50 rounded-xl space-y-2">
                              ...
                              <div className="pt-2 border-t-2 border-gray-200 flex justify-between">
                                ...
                                <span className="font-bold text-primary text-lg">{parseFloat(order.total_amount).toLocaleString('ru-RU')} ₽</span>
                              </div>
                            </div>
                          </div>
                        )}
                        ...
                        {order.comment && (
                          <div className="p-4 bg-blue-50 rounded-xl border-2 border-blue-100">
                            <p className="text-sm font-semibold text-blue-700 mb-2">💬 Комментарий к заказу:</p>
                        ...
                        {order.active_returns && order.active_returns.length > 0 && (
                          <div className="mb-4">
                            <h5 className="text-sm font-semibold text-gray-700 mb-3">🔄 Активные возвраты:</h5>
                            <div className="space-y-2">
                              {order.active_returns.map(returnItem => (
                                <div key={returnItem.id} className="bg-orange-50 border-2 border-orange-200 rounded-xl p-4 space-y-3">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="font-semibold text-gray-900">Возврат {returnItem.return_number}</p>
                                      <p className="text-sm text-gray-600 mt-1">Причина: {returnItem.reason_display}</p>
                                      {returnItem.admin_comment && (
                                        <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded">
                                          <p className="text-xs font-semibold text-blue-700">💬 Комментарий магазина:</p>
                                          <p className="text-sm text-gray-700 mt-1">{returnItem.admin_comment}</p>
                                        </div>
                                      )}
                                      {returnItem.customer_required_action && (
                                        <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded">
                                          <p className="text-xs font-semibold text-red-700">⚠️ Что требуется от вас:</p>
                                          <p className="text-sm text-red-800 mt-1 font-medium">{returnItem.customer_required_action}</p>
                                        </div>
                                      )}
                                      <p className="text-xs text-gray-500 mt-2">Создан: {new Date(returnItem.created_at).toLocaleDateString('ru-RU')}</p>
                                    </div>
                                    <div className="text-right">
                                      {returnItem.status === 'pending' && <span className="inline-block px-4 py-2 bg-orange-500 text-white text-sm font-bold rounded-lg">⏳ На рассмотрении</span>}
                                      {returnItem.status === 'approved' && <span className="inline-block px-4 py-2 bg-green-500 text-white text-sm font-bold rounded-lg">✅ Одобрен</span>}
                                      {returnItem.status === 'rejected' && <span className="inline-block px-4 py-2 bg-red-500 text-white text-sm font-bold rounded-lg">❌ Отклонён</span>}
                                      {returnItem.status === 'need_more_info' && <span className="inline-block px-4 py-2 bg-blue-500 text-white text-sm font-bold rounded-lg">📋 Нужна информация</span>}
                                    </div>
                                  </div>
                    ...
                    {/* Кнопки действий */}
                    <div className="flex flex-wrap gap-3 pt-4 mt-4 border-t-2 border-gray-100">
                      <button
                        onClick={() => toggleOrderDetails(order.id)}
                        className="flex-1 min-w-[200px] btn-primary text-center flex items-center justify-center gap-2"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          {isExpanded ? (
                      ...
                      {isOnlineUnpaid && (
                        <button
                          onClick={() => handleRetryPayment(order)}
                          disabled={retryingPayment === order.id}
                          className="flex-1 min-w-[200px] bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-3 rounded-lg font-semibold transition flex items-center justify-center gap-2 disabled:opacity-50"
                        >
                          {retryingPayment === order.id ? (
                      ...
                      {(order.status === 'pending' || order.status === 'confirmed' || order.status === 'in_progress') && (
                        <button
                          ...
                          className="flex-1 min-w-[200px] bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-lg font-semibold transition flex items-center justify-center gap-2"
                        >
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {order.status === 'awaiting_cancellation' && (
                        <div className="flex-1 min-w-[200px] bg-orange-50 border-2 border-orange-200 px-6 py-3 rounded-lg text-center">
                          <p className="text-orange-700 font-semibold">⏳ Ожидает подтверждения отмены</p>
                        </div>
                      )}
                      ...
                      {order.status === 'delivered' && (
                        <>
                          <button
                            ...
                            className="flex-1 min-w-[200px] bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white px-6 py-3 rounded-lg font-semibold transition flex items-center justify-center gap-2 shadow-lg shadow-green-500/30"
                          >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <button
                            ...
                            className="flex-1 min-w-[200px] bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white px-6 py-3 rounded-lg font-semibold transition flex items-center justify-center gap-2 shadow-lg shadow-orange-500/30"
                          >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setReviewModalOpen(false)}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b-2 border-gray-100 flex items-center justify-between">
              <h3 className="text-2xl font-bold">Выберите товар для отзыва</h3>
              ...
              {selectedOrderForReview.items?.map(item => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition border-2 border-transparent hover:border-primary cursor-pointer group"
                  >
                    <div className="w-20 h-20 bg-white rounded-lg flex-shrink-0 overflow-hidden border-2 border-gray-200">
                      {item.product_image ? (
                        <Image src={item.product_image} alt={item.product_name} width={80} height={80} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-3xl bg-gray-100">📦</div>
                    ...
                    <div className="flex-1 text-left">
                      <p className="font-semibold text-gray-900 group-hover:text-primary transition">{item.product_name}</p>
      ...
      {returnModalOpen && selectedOrderForReturn && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-700">
            <div className="sticky top-0 bg-gradient-to-r from-orange-600 to-red-600 p-6 rounded-t-2xl border-b border-orange-500/30 z-10">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-orange-100 mt-1">Заказ {selectedOrderForReturn.order_number}</p>
                </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={selectAllReturnItems} className="text-sm text-orange-400 hover:text-orange-300 transition font-medium">Выбрать все</button>
                </div>
...
;
                    return (
                      <div
                        ...
                        className={`flex items-center gap-4 p-4 rounded-xl cursor-pointer transition border-2 ${isSelected ? 'bg-orange-500/20 border-orange-500' : 'bg-gray-800/50 border-gray-700 hover:border-gray-600'}`}
                      >
                        <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition ${isSelected ? 'bg-orange-500 border-orange-500' : 'border-gray-600'}`}>
                        ...
                        <div className="w-16 h-16 rounded-lg overflow-hidden bg-gray-900 border border-gray-700 flex-shrink-0">
                          {item.product_image ? (
                ...
                {selectedReturnItems.size === 0 && <p className="text-red-400 text-sm mt-2">Выберите хотя бы один товар</p>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-white mb-3">Причина возврата</label>
                <select value={returnReason} onChange={e => setReturnReason(e.target.value)} className="w-full px-4 py-3 bg-gray-800 border-2 border-gray-700 rounded-xl text-white focus:border-orange-500 focus:outline-none transition">
              ...
              <div>
                <label className="block text-sm font-semibold text-white mb-3">Опишите проблему</label>
                <textarea
                  value={returnDescription}
                  onChange={e => setReturnDescription(e.target.value)}
                  rows={4}
                  placeholder="Расскажите, что не так с товаром..."
                  className="w-full px-4 py-3 bg-gray-800 border-2 border-gray-700 rounded-xl text-white placeholder-gray-500 focus:border-orange-500 focus:outline-none transition resize-none"
                />
                {!returnDescription.trim() && <p className="text-red-400 text-sm mt-2">Пожалуйста, опишите причину возврата</p>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-white mb-3">Фото / видео проблемы (до 5 файлов)</label>
                <input type="file" multiple accept="image/*,video/*" onChange={handleFileChange} className="hidden" id="return-file-upload" />
                <label htmlFor="return-file-upload" className="flex items-center justify-center gap-3 w-full p-6 bg-gray-800/50 border-2 border-dashed border-gray-600 rounded-xl cursor-pointer hover:border-orange-500 hover:bg-gray-800 transition">
                  ...
                  <span className="text-gray-400">Нажмите, чтобы выбрать файлы</span>
                </label>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-4">
                    {returnFiles.map((file, index) => (
                      <div key={index} className="relative group">
                        ...
                        <button onClick={() => removeFile(index)} className="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition shadow-lg">
              ...
              <div className="flex gap-4 pt-4 border-t border-gray-700">
                <button onClick={() => setReturnModalOpen(false)} disabled={submittingReturn} className="flex-1 px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-xl font-semibold transition disabled:opacity-50">Отмена</button>
                <button
                  onClick={handleSubmitReturn}
                  disabled={submittingReturn || selectedReturnItems.size === 0 || !returnDescription.trim()}
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-xl font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-orange-500/30"
                >
                  {submittingReturn ? (
                    <><svg className="animate-spin h-5 w-5" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Отправка...</>
                  ) : (
                    <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Отправить запрос</>
                  )}
                </button>
              </div>
...
function ReturnReplyForm({ returnId, token, onSuccess }: { returnId: number; token: string; onSuccess: () => void }) {
  ...
  return (
    <div className="mt-3 bg-white/70 border border-orange-300 rounded-xl p-4 space-y-3">
      ...
      {files.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {files.map((f, idx) => (
            <div key={idx} className="px-3 py-1.5 bg-orange-100 border border-orange-300 rounded-full text-xs flex items-center gap-2">
              <span className="truncate max-w-[120px]">{f.name}</span>
              <button onClick={() => removeFile(idx)} className="text-red-500 hover:text-red-700 font-bold text-base">×</button>
            </div>
          ))}
        </div>
      )}
      <button onClick={handleSubmit} disabled={loading} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-orange-500 text-white hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed transition">
        {loading ? (
          <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Отправка...</>
        ) : (
          <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Отправить ответ</>
        )}
      </button>
    </div>
  );
