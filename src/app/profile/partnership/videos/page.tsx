...
// SVG иконки соцсетей
const YouTubeIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6" fill="#FF0000">
...
const TelegramIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6" fill="#2AABEE">
    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
  </svg>
);

const VKIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6" fill="#0077FF">
...
const RutubeIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6" fill="#E63934">
...
const OtherLinkIcon = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="#6B7280" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function VideosPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return (
          <span className="flex items-center gap-1 px-2 py-0.5 md:px-3 md:py-1 bg-green-100 text-green-700 rounded-full text-[10px] md:text-xs font-bold">
            <CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5" /> Одобрено
          </span>
        );
      case 'pending':
        return (
          <span className="flex items-center gap-1 px-2 py-0.5 md:px-3 md:py-1 bg-yellow-100 text-yellow-700 rounded-full text-[10px] md:text-xs font-bold">
            <Clock className="w-3 h-3 md:w-3.5 md:h-3.5" /> На модерации
          </span>
        );
      case 'rejected':
        return (
          <span className="flex items-center gap-1 px-2 py-0.5 md:px-3 md:py-1 bg-red-100 text-red-700 rounded-full text-[10px] md:text-xs font-bold">
            <XCircle className="w-3 h-3 md:w-3.5 md:h-3.5" /> Отклонено
          </span>
        );
      default:
        return null;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-4 md:py-8 max-w-6xl">
        ...
        <div className="mb-6 md:mb-8">
          <h1 className="text-2xl md:text-4xl font-bold mb-1 md:mb-2">Мои видео</h1>
          <p className="text-sm md:text-base text-gray-600">Загружайте видео с обзорами товаров</p>
        </div>
        ...
        <motion.button
          ...
          className="bg-purple-600 text-white px-4 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl font-bold mb-6 md:mb-8 flex items-center gap-2 hover:bg-purple-700 transition text-sm md:text-base"
        >
        ...
        {showForm && (
          ...
          >
            <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">Загрузить видео</h2>
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5 md:gap-2">
                  {PLATFORMS.map(({ value, label, Icon }) => (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-1 px-1 py-2 md:px-2 md:py-3 rounded-lg md:rounded-xl border-2 text-[10px] md:text-xs font-semibold transition
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-3 py-2 md:px-4 md:py-3 border-2 border-gray-200 rounded-lg md:rounded-xl focus:border-purple-400 outline-none transition text-sm md:text-base"
                />
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 md:gap-3 pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-purple-600 text-white px-4 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl font-bold hover:bg-purple-700 disabled:opacity-60 transition flex items-center justify-center gap-2 text-sm md:text-base"
                >
                  {submitting ? (
                    <>
                ...
                <button
                  ...
                  className="bg-gray-200 text-gray-700 px-4 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl font-bold hover:bg-gray-300 transition text-sm md:text-base"
                >
                  Отмена
                </button>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {videos.length === 0 ? (
            <div className="col-span-2 bg-white rounded-xl md:rounded-2xl p-8 md:p-12 text-center shadow-xl">
              <Video className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 md:mb-4 text-gray-300" />
              <p className="text-gray-500 text-base md:text-lg">У вас пока нет загруженных видео</p>
            </div>
          ) : (
            videos.map(video => {
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-white rounded-xl md:rounded-2xl overflow-hidden shadow-lg"
                >
                  <div className="p-4 md:p-6">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-3 md:mb-4">
                      <div className="flex items-center gap-2 md:gap-3">
                        <div className="w-8 h-8 md:w-10 md:h-10 bg-gray-50 rounded-lg md:rounded-xl flex items-center justify-center border border-gray-100">
                          <PlatformIcon />
                        </div>
                        <div>
                    ...
                    <a
                      ...
                      className="flex items-center gap-1.5 text-purple-600 hover:underline text-xs md:text-sm break-all"
                    >
                    ...
                    {video.admin_notes && (
                      <div className="mt-3 bg-blue-50 border border-blue-100 rounded-lg md:rounded-xl p-2 md:p-3">
                        <p className="text-[10px] md:text-xs font-semibold text-blue-700 mb-1 flex items-center gap-1">
                          💬 Комментарий администратора
                        </p>
                        <p className="text-xs md:text-sm text-blue-800 break-words">{video.admin_notes}</p>
                      </div>
                    )}

                    {video.rejection_reason && (
                      <p className="mt-2 md:mt-3 text-[10px] md:text-xs text-red-600 bg-red-50 rounded-lg p-1.5 md:p-2 break-words">
