...
export default function ReviewList({ productId }: ReviewListProps) {
  ...
  const fetchReviews = async () => {
    try {
      ...
      else if (data?.results) setReviews(data.results);
      else setReviews([]);
    } catch (error) {
  ...
  const getFilteredReviews = () => {
    ...
    if (sortBy === 'helpful') filtered.sort((a, b) => b.helpful_count - a.helpful_count);
  ...
  return (
    <div>
      {stats && stats.count > 0 && (
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-4 md:p-8 mb-6 md:mb-8">
              ...
              {[5, 4, 3, 2, 1].map((rating) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 md:gap-3 p-2 rounded-lg transition ${
                      filterRating === rating ? 'bg-white shadow-md' : 'hover:bg-white/50'
                    }`}
                  >
                    <span className="text-xs md:text-sm font-medium w-8">{rating} ★</span>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-2 py-1.5 md:px-3 md:py-1.5 text-sm focus:ring-2 focus:ring-primary"
          >
        ...
        {filteredReviews.map((review) => {
          ...
          return (
            <div 
              id={`review-${review.id}`}
              key={review.id} 
              className="bg-white rounded-xl p-4 md:p-6 shadow-md hover:shadow-lg transition scroll-mt-20"
            >
              <div className="flex items-start gap-3 md:gap-4 mb-3 md:mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-1 md:gap-2 mb-1">
                    <h4 className="font-bold text-base md:text-lg text-gray-900 truncate">
                      {review.author_name || review.author_username || 'Аноним'}
                    </h4>
                    {review.author_tags && review.author_tags.length > 0 && (
                      <UserBadges badges={review.author_tags} size="small" maxDisplay={3} />
                    )}
                    {review.is_verified && (
                      <span className="bg-green-100 text-green-800 text-[10px] md:text-xs px-1.5 py-0.5 md:px-2 md:py-1 rounded-full font-medium flex items-center gap-1">
                  ...
                  <p className="text-xs md:text-sm text-gray-500 mb-1 md:mb-2">{formatDate(review.created_at)}</p>
                  <StarRating rating={review.rating} size="sm" />
                </div>
              </div>

              {review.title && (
                <h5 className="font-bold text-base md:text-lg text-gray-900 mb-2 md:mb-3">{review.title}</h5>
              )}

              <p className="text-gray-700 leading-relaxed mb-3 md:mb-4 text-sm md:text-base break-words">{review.comment}</p>
              ...
              {review.media && review.media.length > 0 && (
                <div className="flex gap-2 mb-3 md:mb-4 overflow-x-auto pb-2">
                  {review.media.map((item) => (
                    <div
                      key={item.id}
                      className="relative flex-shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-lg overflow-hidden cursor-pointer group shadow-md hover:shadow-xl transition"
              ...
              {(review.pros || review.cons) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-3 md:mb-4 p-3 md:p-4 bg-gray-50 rounded-lg">
                  {review.pros && (
                    <div>
                      <h6 className="font-semibold text-green-700 mb-1 md:mb-2 flex items-center gap-2 text-sm">
                        <span>✓</span> Достоинства:
                      </h6>
                      <p className="text-xs md:text-sm text-gray-700 break-words">{review.pros}</p>
                    </div>
                  )}
                  {review.cons && (
                    <div>
                      <h6 className="font-semibold text-red-700 mb-1 md:mb-2 flex items-center gap-2 text-sm">
                        <span>✕</span> Недостатки:
                      </h6>
                      <p className="text-xs md:text-sm text-gray-700 break-words">{review.cons}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {review.replies && review.replies.length > 0 && (
                <div className="mt-3 md:mt-4 space-y-2 md:space-y-3 border-t pt-3 md:pt-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-1 md:gap-2 mb-1">
                              ...
                              {reply.author_tags && reply.author_tags.length > 0 && (
                                <UserBadges badges={reply.author_tags} size="small" maxDisplay={2} />
                              )}
                              {reply.is_official && (
                                <span className="text-[10px] md:text-xs bg-blue-600 text-white px-1.5 py-0.5 rounded-full font-medium">
                                  Официальный ответ
                                </span>
                              )}
                            </div>
                            <p className="text-gray-700 text-xs md:text-sm whitespace-pre-wrap break-words">{reply.text}</p>
              ...
              {isAuthenticated && replyingToReview !== review.id && (
                <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t mb-3 md:mb-4">
                  <button
                    onClick={() => setReplyingToReview(review.id)}
                    className="w-full sm:w-auto group inline-flex items-center justify-center gap-2 px-4 py-2 md:px-5 md:py-3 bg-white border border-gray-200 hover:border-primary text-gray-700 hover:text-primary font-medium rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 text-sm"
                  >
                    <div className="w-6 h-6 md:w-8 md:h-8 bg-blue-50 group-hover:bg-blue-100 rounded-full flex items-center justify-center transition-colors">
                      <MessageSquareIcon size={14} />
                    </div>
                    <span>Ответить на отзыв</span>
                    <svg 
                      className="w-3 h-3 md:w-4 md:h-4 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-1 transition-all" 
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
              ...
              {replyingToReview === review.id && (
                <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t bg-gray-50 rounded-lg p-3 md:p-4">
                  <ReviewReplyForm
                    reviewId={review.id}
                    onSuccess={() => {
              ...
              <div className="flex flex-wrap items-center gap-3 md:gap-4 pt-3 md:pt-4 border-t">
                <span className="text-xs md:text-sm text-gray-600">Полезен отзыв?</span>
                <button
                  onClick={() => handleHelpful(review.id)}
                  disabled={votedReviews.has(review.id)}
                  className={`flex items-center gap-1 md:gap-2 px-2 py-1 md:px-3 md:py-1.5 rounded-lg transition text-xs md:text-sm font-medium ${
      ...
      {lightboxOpen && lightboxImage && (
        ...
        >
          <button
            onClick={closeLightbox}
            className="absolute top-2 right-2 md:top-4 md:right-4 text-white hover:text-gray-300 transition z-10 bg-black bg-opacity-50 rounded-full p-1.5 md:p-2"
          >
