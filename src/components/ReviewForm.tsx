...
export default function ReviewForm({ productId, onSuccess }: ReviewFormProps) {
  const { user, tokens } = useAuth(); // ✅ Получаем пользователя
  ...
  // ✅ АВТОЗАПОЛНЕНИЕ при загрузке компонента
  useEffect(() => {
    if (user) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-8">
      <h3 className="text-2xl font-bold mb-6 text-gray-900">Написать отзыв</h3>
      ...
      {/* ✅ УВЕДОМЛЕНИЕ ДЛЯ АВТОРИЗОВАННЫХ */}
      {user && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div>
          ...
          <input
            ...
            disabled={!!user} // ✅ Блокируем для авторизованных
            className={`w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent ${
...
<div className="mb-6">
  ...
  <textarea
    ...
    className={`w-full border rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent resize-none ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <textarea
            ...
            className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent resize-none"
